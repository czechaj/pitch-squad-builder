...
const positionAttributeWeights: Record<Position, Partial<Record<AttributeKey, number>>> = {
...
function positionalScore(player: Player, position: Position): number {
  ...
  for (const [attr, weight] of Object.entries(weights)) {
    score += player.attributes[attr as AttributeKey] * (weight ?? 0);
  }
  return score;
}
...
function balanceTeams(players: Player[], teamCount: number): TeamResult[] {
  const teams: TeamResult[] = Array.from({ length: teamCount }).map((_, i) => ({
...
app.post("/api/balance", (req, res) => {
  ...
  return res.json({
    teams,
    players: players.map((p) => ({ id: p.id, name: p.name, power: Number(playerPower(p as Player).toFixed(2)) }))
  });
});
